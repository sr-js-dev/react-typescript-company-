...
export class Benefit extends React.Component<IBenefitProps, IBenefitState> {
  ...
  render() {
    const { benefitList, match, totalItems } = this.props;
    return (
      <div>
        ...
        <div className="table-responsive">
          {benefitList && benefitList.length > 0 ? (
            <Table responsive>
              <thead>
                <tr>
                  ...
                  <th>
                    Benefit Type <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {benefitList.map((benefit, i) => (
                  <tr key={`entity-${i}`}>
                    <td>
                      <Button tag={Link} to={`${match.url}/${benefit.id}`} color="link" size="sm">
                        {benefit.id}
                      </Button>
                    </td>
                    <td>{benefit.name}</td>
                    <td>{benefit.benefitRate}</td>
                    <td>{benefit.description}</td>
                    <td>
                      {benefit.benefitTypeId ? <Link to={`benefit-type/${benefit.benefitTypeId}`}>{benefit.benefitTypeId}</Link> : ''}
                    </td>
                    <td className="text-right">
                      <div className="btn-group flex-btn-group-container">
                        <Button tag={Link} to={`${match.url}/${benefit.id}`} color="info" size="sm">
                          <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                        </Button>
                        <Button tag={Link} to={`${match.url}/${benefit.id}/edit`} color="primary" size="sm">
                          <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                        </Button>
                        <Button tag={Link} to={`${match.url}/${benefit.id}/delete`} color="danger" size="sm">
            ...
            <div className="alert alert-warning">No Benefits found</div>
          )}
        </div>
        <div className={benefitList && benefitList.length > 0 ? '' : 'd-none'}>
